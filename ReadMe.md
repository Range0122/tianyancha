...
### 17.05.02
最近一直在更新代码，ReadMe没有更新
* 对企业年报、招投标书、法律诉讼的详细信息进行了抓取
* if-zz在`pipelines`中添加了插入数据库的语句
* 对所有字段进行了插入数据库的测试，多次根据数据类型修改了数据库
* 在Linux上部署了爬虫，主要是针对文件路径的一些简单修改，phantomjs和一些packages的安装也都比较简单
* 修改了多处抓取企业年报详细信息的逻辑错误
* `item`新增`page`字段用以翻页逻辑
* 在某代理那里买了一些ip，使用对方给出的api编写了`get_ip.py`定时更新ip池，但由于ip质量较差，达不到实际使用的要求
* `middleware.py`中新增了从ip池中调用ip的函数，防止本地ip被ban掉
* 新增`safe_append`系列的函数，用于代码的重构
* 在循环语句中使用`names=global()`的方法进行代码重构
* 目前刚刚开始代码重构，比较繁琐，并且还需要进行测试

### 17.05.12
* 之前将图片存入数据库中的方式是将url存入，现在改为保存在Linux本地
* 针对天眼查的html结构变动再次更新`spider`