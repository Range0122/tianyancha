...
class TianyanchaPipeline(object):
        ...
        if len(memory[0]) > 0:
         for i in xrange(0, len(memory[0])):
            try:
                ...
                print (db_name+tuple(x[i] for x in memory) + "insert not success")
         self.conn.commit()
    ...
    def process_item(self, item, spider):
        ...
        for i in xrange(len(self.company)):
            if item[self.company[i]]==u'未公开':
                item[self.company[i]]=None
          ...
          for (x, y, i) in zip(item["shareholder_name"], item["shareholder_id"], xrange(len(memory[0]))):
           try:
            if len(x) > 4 and x.find(u'公司'):
                ...
                if selec_result == 0:
                    self.cursor.execute("INSERT INTO company(tyqycid,comp_name) "
                                        "VALUES (%s,%s)", (y, x))
