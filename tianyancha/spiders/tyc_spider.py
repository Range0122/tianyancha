...
class TianYanCha_Spider(CrawlSpider):
    ...
    def parse_page(self, response):
        try:
            ...
            company_name = response.selector.xpath('//div[@class="company_info_text"]/div[1]/text()' or
                                                   '//span[@class="base-company f16 ng-binding"]/text()').extract()[0]
            legal_representative = response.selector.xpath(u'//div[text()="法定代表人"]/following-sibling::div[1]/a/text()' or
                                                           '//a[@ng-if="company.baseInfo.legalPersonName"]/text()').extract()[0]
            registered_capital = response.selector.xpath(u'//div[text()="注册资本"]/following-sibling::div[1]/text()' or
                                                         '//td[@class="td-regCapital-value"]/p/text()').extract()[0]
            registered_time = response.selector.xpath(u'//div[text()="注册时间"]/following-sibling::div[1]/text()' or
                                                      '//td[@class="td-regTime-value"]/p/text()').extract()[0]
            condition = response.selector.xpath(u'//div[text()="状态"]/following-sibling::div[1]/text()' or
            ...
            registered_address = response.selector.xpath('//td[@class="basic-td ng-scope"]/div/span/text()').extract()[0]
            business_scope = response.selector.xpath('//td[@class="basic-td ng-scope"]/div/span/span/text()').extract()[0]
            ...
            website = response.selector.xpath('//div[@class="company_info_text"]/span[3]/a/text()' or
                                              '//div[@class="company_info_text"]/span[3]/span[2]/text()' or
                                              '//span[@ng-hide="company.websiteList"]/text()').extract_first(default=u'暂无')
            score = response.selector.xpath('//td[@class="td-score position-rel"]/img/@ng-alt' or
            ...
            former_name = response.selector.xpath(u'//span[text()="曾用名"]/following-sibling::span[2]/text()').extract()
            if not former_name:
                former_name = []
